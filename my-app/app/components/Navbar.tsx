...
export default function Navbar() {
  ...
  return (
    <div className="relative">
      <nav className="fixed bg-[#402E32] w-full py-5 shadow-xl text-slate-100 z-40 shadow-md">
        <div className="flex justify-between items-center px-7 2xl:px-16">
          <div className="flex">
            ...
            <Link href="/" className="font-bold text-xl mx-5">
              PLANTIEW
            </Link>
          </div>
          <div className="flex items-center hidden lg:block">
            <ul className="flex items-center">
              <li className="ml-2">
                <Link
                  href="/"
                  className="hover:text-[#4E3C05] hover:bg-[#F5F0E8] px-5 py-1 rounded-full duration-100"
                >
                  Home
                </Link>
              </li>
              ...
              {status == "authenticated" ? (
                <>
                  ...
                  <li className="dropdown ml-2 flex items-center relative">
                    <button
                      onClick={() => setDropDown((prevDisplay) => !prevDisplay)}
                      type="button"
                    ...
                    <div
                      className={
                        (dropDownShow
                          ? "md:block md:right-0 md:top-8 min-w-60"
                          : "md:hidden") +
                        " dropdown-body md:absolute p-3 bg-white rounded-md shadow text-slate-600"
                      }
                    >
                      <ul>
                        <li>
                          <Link
                            href="/profile"
                            className="block flex items-center hover:bg-slate-100 py-1.5 px-3 rounded-md"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="1.25em"
                        ...
                        <hr className="my-3 h-px bg-slate-400 border-none" />
                        <li>
                          <button
                            className="w-full block px-3 py-1.5 bg-red-600 text-white hover:bg-red-500 text-whtie rounded-md duration-100"
                            onClick={() => signOut()}
                          >
                            Sign out
                          </button>
                ...
                <li className="ml-2">
                  <Link
                    href="/auth/signin"
                    className="bg-white text-slate-600 font-bold px-5 py-1 rounded-full duration-100"
                  >
                    Sign In
                  </Link>
                </li>
      ...
      <div
        className={
          "sidebar fixed h-screen bg-[#39272B] py-6 px-5 text-slate-100 duration-500 min-w-60 overflow-auto top-14 shadow-md " +
      ...
      >
        <div className="sidebar-header mt-3 block">
          {status == "authenticated" ? (
            ...
            <Link
              href="/auth/signin"
              className="block w-full bg-[#674F04] hover:bg-[#7C6417] px-7 py-1.5 rounded-md text-center duration-100"
            >
              Sign In
            </Link>
          )}
        ...
        <div className="sidebar-body">
            ...
            <div className="sidebar-section-body">
              <ul>
                <li>
                  <Link
                    href="/"
                    className="block flex items-center mb-3 hover:bg-[#F5F0E8] py-2 px-5 rounded-md hover:text-[#4E3C05]"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="1.25em"
          ...
          <div className="sidebar-section">
            {status == "authenticated" && (
              <button
                className="w-full block px-5 py-1.5 bg-red-600 hover:bg-red-500 text-whtie rounded-md duration-100"
                onClick={() => signOut()}
              >
                Sign out
              </button>
